...
config.set_main_option(
    "sqlalchemy.url",
    os.getenv("CATALOG_DATABASE_URL", config.get_main_option("sqlalchemy.url")),
)
target_metadata = Base.metadata

# alembic.ini is the single source of truth for the version-table location.
version_table = config.get_main_option("version_table", "alembic_version_catalog")
version_table_schema = config.get_main_option("version_table_schema", "catalog")


def include_name(name: str | None, type_: str, parent_names: dict[str, str | None]) -> bool:
    """Keep Catalog autogeneration blind to tables owned by other services."""
