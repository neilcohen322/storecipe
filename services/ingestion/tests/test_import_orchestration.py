...
from ingestion.orchestration import ImportOrchestrator, StaleLease
...
async def test_claim_excludes_a_concurrent_worker_and_recovers_with_a_new_generation(
    ...
    orchestration = ImportOrchestrator(ImportRepository(session))
...
async def test_fenced_mutations_reject_stale_workers_and_terminal_jobs(
    ...
    assert await orchestration.finish_terminal(token, ImportStatus.COMPLETED)
    with pytest.raises(StaleLease):
...
async def test_stage_checkpoints_are_persisted_with_legal_progression(
    ...
    assert await orchestration.advance_stage(token, ImportStage.FETCHING)
...
async def test_retry_atomically_fences_the_old_worker_and_creates_the_next_dispatch(
    ...
    with pytest.raises(StaleLease):
        await orchestration.advance_stage(token, ImportStage.FETCHING)
...
async def test_cancellation_blocks_retry_and_catalog_reservation(session: AsyncSession) -> None:
    ...
    assert not await orchestration.schedule_retry(token, datetime.now(UTC) + timedelta(minutes=1))
...
async def test_catalog_intent_reserves_one_idempotent_attempt(session: AsyncSession) -> None:
    ...
    assert token is not None
    assert await orchestration.advance_stage(token, ImportStage.FETCHING)
    assert await orchestration.advance_stage(token, ImportStage.EXTRACTING)
    assert await orchestration.advance_stage(token, ImportStage.MODEL_EXTRACTING)
    assert await orchestration.advance_stage(token, ImportStage.VALIDATING)
