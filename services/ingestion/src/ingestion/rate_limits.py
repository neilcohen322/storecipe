...
class RedisBurstLimiter:
    ...
    Redis failures deliberately fail open. The adapter logs only a stable event name;
    ...
    async def hit(self, subject: str, operation: str) -> RateLimitDecision:
        ...
        try:
            ...
            logger.info("rate_limit.degraded")
            return RateLimitDecision(
                allowed=True,
                limit=self._amount,
...
class UnlimitedBurstLimiter:
    """Deterministic no-op limiter for tests and explicit degraded operation."""
