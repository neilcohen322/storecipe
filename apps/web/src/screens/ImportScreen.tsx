...
export function ImportScreen({ onBack }: ImportScreenProps) {
  ...
  return <Screen>
    ...
    {session.terminalSummary ? <View accessibilityLiveRegion="polite"><Text>{terminalCopy(session.terminalSummary.status)}</Text>{session.terminalSummary.canRetry ? <Button label="Retry import" variant="secondary" onPress={() => void session.retryImport()} /> : null}</View> : null}
