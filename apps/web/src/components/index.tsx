...
export const Button = forwardRef<React.ElementRef<typeof Pressable>, ButtonProps>(function Button({ label, variant = "primary", loading = false, disabled, icon, style, onPress, onFocus, onBlur, onPointerDown, accessibilityState, ...props }, ref) {
  ...
  const hoverBg = variant === "primary" ? colors.accentHover : bg;
  return <Pressable {...props} ref={ref} onPointerDown={(event) => { modality.markPointer(); onPointerDown?.(event); }} onHoverIn={() => setHovered(true)} onHoverOut={() => setHovered(false)} onPress={disabled || loading ? undefined : onPress} onFocus={(event) => { setFocusVisible(matchesFocusVisible(event.currentTarget) ?? modality.isKeyboard()); onFocus?.(event); }} onBlur={(event) => { setFocusVisible(false); onBlur?.(event); }} accessibilityRole="button" accessibilityLabel={label} accessibilityState={{ ...accessibilityState, disabled: disabled || loading, busy: loading }} disabled={disabled || loading} focusable={!(disabled || loading)} style={(state) => { const customStyle = typeof style === "function" ? style(state) : style; return [styles.button, { backgroundColor: hovered ? hoverBg : bg, borderColor: variant === "quiet" ? "transparent" : colors.border, opacity: state.pressed ? 0.76 : 1 }, variant === "icon" && styles.iconButton, customStyle, focusVisible && webFocus(colors.focusRing)]; }}><>{loading ? <ActivityIndicator color={fg} /> : icon}{variant !== "icon" && <Text style={[styles.buttonText, { color: fg, fontSize: theme.type.body }]}>{label}</Text>}</></Pressable>;
});
...
export function Screen({ children, ...props }: ComponentProps<typeof ScrollView>) {
  ...
  return (
    <ScrollView
      {...props}
      style={[{ flex: 1, width: "100%", minHeight: 0, backgroundColor: theme.colors.canvas }, props.style]}
      contentContainerStyle={[styles.screen, { paddingTop: insets.top + theme.spacing.md, paddingRight: insets.right + theme.spacing.md, paddingBottom: insets.bottom + theme.spacing.md, paddingLeft: insets.left + theme.spacing.md }, props.contentContainerStyle]}
    >
      <View testID="screen-content" style={[styles.readable, { maxWidth: 1120, width: "100%" }]}>{children}</View>
    </ScrollView>
  );
}
...
export function ErrorState({ title = "Something went wrong", description, action }: { title?: string; description?: string; action?: ReactNode }) {
  const { theme } = useTheme();
  return (
    <View testID="error-state" accessibilityRole="alert" style={[styles.errorState, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
      <Text style={[styles.errorLabel, { color: theme.colors.danger }]}>Error</Text>
      <Text accessibilityRole="header" style={[styles.stateTitle, { color: theme.colors.text }]}>{title}</Text>
      {description ? <Text style={[styles.hint, { color: theme.colors.mutedText }]}>{description}</Text> : null}
      {action}
    </View>
  );
}
...
export { RecipeMedia } from "./RecipeMedia";
export { AuthenticatedRecipeImage } from "./AuthenticatedRecipeImage";

const styles = StyleSheet.create({
  button: { minHeight: 44, minWidth: 44, borderWidth: 1, borderRadius: 12, paddingHorizontal: 16, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8 },
  iconButton: { width: 44, paddingHorizontal: 0 },
  buttonText: { fontWeight: "600" },
  field: { gap: 8, marginBottom: 16 },
  fieldControl: { minHeight: 44, minWidth: 44, borderWidth: 1, borderRadius: 8, paddingHorizontal: 12 },
  label: { fontSize: 15, fontWeight: "600" },
  hint: { fontSize: 12 },
  textarea: { minHeight: 120, minWidth: 44, borderWidth: 1, borderRadius: 8, padding: 12 },
  screen: { flexGrow: 1, alignItems: "stretch" },
  readable: { width: "100%", maxWidth: 1120, alignSelf: "center" },
  ...
  errorState: { width: "100%", minHeight: 44, padding: 20, alignItems: "flex-start", justifyContent: "center", gap: 8, borderWidth: 1, borderRadius: 16 },
  errorLabel: { fontSize: 12, fontWeight: "700" },
  stateTitle: { fontSize: 18, fontWeight: "700" },
  badge: { minHeight: 44, minWidth: 44, paddingHorizontal: 12, borderWidth: 1, borderRadius: 999, alignItems: "center", justifyContent: "center" },
  rating: { flexDirection: "row" },
  ratingButton: { minWidth: 44, minHeight: 44, alignItems: "center", justifyContent: "center" },
  toast: { minHeight: 44, padding: 12, borderRadius: 8 },
  scrim: { flex: 1, alignItems: "center", justifyContent: "center", padding: 24 },
  dialog: { width: "100%", maxWidth: 480, gap: 16, padding: 24, borderRadius: 16 },
  actions: { flexDirection: "row", justifyContent: "flex-end", gap: 8 },
  media: { minHeight: 160, alignItems: "center", justifyContent: "center", borderRadius: 16 },
  mediaText: { fontSize: 36, fontWeight: "800" },
});
