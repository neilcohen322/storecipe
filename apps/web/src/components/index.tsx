...
export const Button = forwardRef<React.ElementRef<typeof Pressable>, ButtonProps>(function Button({ label, variant = "primary", loading = false, disabled, icon, style, onPress, onFocus, onBlur, onPointerDown, accessibilityState, ...props }, ref) {
  ...
  return <Pressable {...props} ref={ref} onPointerDown={(event) => { modality.markPointer(); onPointerDown?.(event); }} onHoverIn={() => setHovered(true)} onHoverOut={() => setHovered(false)} onPress={disabled || loading ? undefined : onPress} onFocus={(event) => { setFocusVisible(matchesFocusVisible(event.currentTarget) ?? modality.isKeyboard()); onFocus?.(event); }} onBlur={(event) => { setFocusVisible(false); onBlur?.(event); }} accessibilityRole="button" accessibilityLabel={label} accessibilityState={{ ...accessibilityState, disabled: disabled || loading, busy: loading }} disabled={disabled || loading} focusable style={(state) => { const customStyle = typeof style === "function" ? style(state) : style; return [styles.button, { backgroundColor: hovered ? hoverBg : bg, borderColor: variant === "quiet" ? "transparent" : colors.border, opacity: state.pressed ? 0.76 : 1 }, variant === "icon" && styles.iconButton, customStyle, focusVisible && webFocus(colors.focusRing)]; }}><>{loading ? <ActivityIndicator color={fg} /> : icon}{variant !== "icon" && <Text style={[styles.buttonText, { color: fg, fontSize: theme.type.body }]}>{label}</Text>}</></Pressable>;
});
