...
export function AppShell({ children, viewportWidth }: PropsWithChildren<{ viewportWidth?: number }>) {
  ...
  if (mode === "compact") return <View testID="app-shell-compact" style={[styles.compact, { backgroundColor: theme.colors.canvas, paddingBottom: insets.bottom + COMPACT_NAVIGATION_HEIGHT }]}><View style={[styles.content, styles.compactContent]}>{children}</View><BottomNavigation /></View>;
...
const styles = StyleSheet.create({ compact: { flex: 1 }, compactContent: { flex: 1 }, desktop: { flex: 1, flexDirection: "row" }, canvas: { flex: 1, alignItems: "center" }, content: { width: "100%", maxWidth: 1120 }, pageAction: { width: "100%", maxWidth: 1120, alignItems: "flex-end", marginBottom: 16 }, createAction: { minWidth: 44, minHeight: 44, borderRadius: 12, alignItems: "center", justifyContent: "center" } });
