...
function Placeholder({ title, tags = [] }: { title: string; tags?: string[] }) {
  ...
  const colors = [theme.colors.accent, theme.colors.success, theme.colors.warning, theme.colors.danger];
  return (
    <View
      testID="recipe-media"
    ...
    >
      <Text style={{ fontSize: 36, fontWeight: "800", color: theme.colors.accentContrast }}>{initials}</Text>
    </View>
  );
}
